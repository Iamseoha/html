					...
					if(result.error == 0) {
						if(/\.(gif|jpe?g|png|webp)$/i.test(result.source_filename) && opt.autoinsertTypes.image) {
							...
							if(result.original_type === 'image/gif') {
								...
								temp_code += '<video src="' + result.download_url + '" controls playsinline preload="none" data-file-srl="' + result.file_srl + '" />';
							} else {
								temp_code += '<video src="' + result.download_url + '" controls playsinline data-file-srl="' + result.file_srl + '" />';
							}
